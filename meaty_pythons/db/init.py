...
def load_data_to_redis(file_path, redis_connection):
    with open(file_path, "r") as csv_file:
        ...
        for row in reader:
            ...
            redis_connection.hset(row["id"], mapping=row)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Redis server host and port")
    parser.add_argument(
        "--host", default=config.get("REDIS", "HOST"), help="host to listen on"
    )
    parser.add_argument(
        "--port", default=config.get("REDIS", "PORT"), help="port to listen on"
    )
